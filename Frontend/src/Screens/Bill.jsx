...
export default function Bill() {
  ...
  // };
    ...
    let lastInvoice = await axios.get("http://localhost:3000/bill/lastBill");

    const last_id = lastInvoice.data.bill_id ;
      
    if (last_id) {
      const match = last_id.match(/^SS([A-Z]+)(\d{8})$/);
      if (match) {
        letterPart = match[1];
  ...
  //localStorage.setItem("lastInvoice", newInvoice);
